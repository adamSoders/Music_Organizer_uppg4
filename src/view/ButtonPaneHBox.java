...
public class ButtonPaneHBox extends HBox {
	...
	public ButtonPaneHBox(MusicOrganizerController contr, MusicOrganizerWindow view) {
		...
		newWindowButton = createNewWindowButton(); //skapar nya knappen
	...
	private Button createPlaySoundClipsButton() {
		...
		button.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				controller.playSoundClips(view.getSelectedSoundClips()); // Modified to take selected soundclips from view (according to current requirements only the main window can use the button) 
			}
	...
	private Button createNewWindowButton() { // Creates button to open selected album in new window
		Button button = new Button("New Window");
		button.setTooltip(new Tooltip("Opens selected album in new window"));
		button.setMinWidth(BUTTON_MIN_WIDTH);
		button.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				controller.createAlbumWindow(view.getSelectedAlbum()); // Controller creates albumwindow and notifies manager
			}
